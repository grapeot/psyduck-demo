...
for(const x of [.92,.96,1,1.04,1.08,1.12,1.16,1.20])for(const y of [2.44,2.46,2.48,2.50,2.52,2.54,2.56]){
  ...
  const valid=Number.isFinite(angle)&&angle>=.12&&angle<=2.9&&bend>=0&&bend<=armBind.maxElbow;
...
const output=await mkdtemp('test-results/pose-candidates-');
await writeFile(`${output}/candidates.json`,JSON.stringify({purpose:'单轮参数可行性筛选，不改几何/权重，不渲染成多轮；头部椭球代理阈值保持 1。',armBind,assetSha256:createHash('sha256').update(bytes).digest('hex'),candidates},null,2));
console.log(output,candidates.filter(c=>c.withinExistingLimits&&c.distalInsideProxy===0&&c.distalInsideSculptHead===0));
